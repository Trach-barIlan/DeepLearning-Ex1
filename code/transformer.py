...
class Embed(nn.Module):
    ...
    def forward(self, x):
        ...
        B, N = x.size()
        tok_embeddings = self.token_embeddings(x)  # (b x n x embed_size)
        
        positions = torch.arange(N, device=x.device)
        pos_embeddings = self.position_embeddings(positions)  # (n x embed_size)
...
class TransformerLM(nn.Module):
    ...
    def init_weights(self):
        # initialize weights by module type
